...
export default function AllMatches() {
  ...
  const groupedMatches = useMemo(() => {
    ...
    filteredMatches.forEach(match => {
      try {
        const dateStr = format(new Date(match.date), "dd 'de' MMMM, yyyy", { locale: ptBR });
        if (!groups[dateStr]) {
          groups[dateStr] = [];
        }
                        ...
                        <div className="flex-1 text-center min-w-0">
                          ...
                          <div className="flex flex-col space-y-1">
                            {match.blueTeam.map(id => (
                              <span key={id} className="text-xs font-semibold text-gray-700 truncate block px-1" title={athletes[id]?.name || 'Atleta'}>
                                {athletes[id]?.name || '...'}
                              </span>
                            ))}
                          </div>
